  ...
  return (
    <>
        ...
        <div className="flex items-center justify-between w-[90%] mx-auto">
          ...
          <div className="w-[60%] flex items-center">
            ...
            <div
              ...
              className="bg-[#FEBD69] rounded-r-md p-2"
            >
      ...
      <div className="bg-[#232F3E] w-full text-white p-2 flex justify-between items-center">
          ...
          {itemList.map((link, idx) => {
            return (
              <Link
                href={`/${link}`}
                key={idx}
                className="mx-2 hover:border border border-transparent hover:border-white"
              >
                {link}
              </Link>
            );
        ...
        <div className="mr-5" onClick={async()=> {
          const {error} = await supabase.auth.signOut()
          router.push('/signin')
        }}>
          <h1 className="font-bold text-[#FEBD69] cursor-pointer hover:underline">Sign out</h1>
        </div>
      </div>
    </>
  );
